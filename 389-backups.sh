...
######################################################################################
# NAME: 389-backup.sh
# PURPOSE:This script backups the 389 Directory Server Database and takes and LDIF
# export of the DS.
# It stores these backups in /var/dirsrv/backups/ldap.<TIMESTAMP>/
# When run, it removes directories whose TIMESTAMP is greater than the ARCHIVE_PERIOD set.
########################################################################################
#########################################################################################
...
[ ! -d "${BACKUP_DIR}/ldap.${TIMESTAMP}" ] && ${ECHO} mkdir -m 0750 -p ${BACKUP_DIR}/ldap.${TIMESTAMP}
${ECHO} chmod -R 750 ${BACKUP_DIR}
${ECHO} chown -R nobody:nobody ${BACKUP_DIR}

# Extract an LDIF from the current LDAP configuration.
${ECHO} /usr/sbin/db2ldif -Z ${HOST} -n userRoot -a ${BACKUP_DIR}/ldap.${TIMESTAMP}/userRoot.ldif -U
...
for DIR in $(find /var/lib/dirsrv/slapd-${HOST}/bak -type d -name "${HOST}-*" -mtime +${ARCHIVE_PERIOD} ); do
  ${ECHO} rm -rf ${DIR}
done
